...
export const readAlbums: RequestHandler = async (req: Request, res: Response) => {
  try {
    ...
    if (Number.isNaN(albumId)) {
      albums = await AlbumDao.readAlbums(); // Call dao to get all albums
    } else {
      albums = await AlbumDao.readAlbumsByAlbumId(albumId);
    }

    await readTracks(albums, res); // Get tracks for each album

    res.status(200).json(albums); //Send albums as response
